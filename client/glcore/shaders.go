...
type TShaderManager struct {
	...
	AllSources *TShaderSources
}

func NewShaderManager () *TShaderManager {
	var sm = &TShaderManager {}
	sm.AllProgs = map[string]*glutil.TShaderProgram {}
	return sm
}

func (me *TShaderManager) dispose () {
...
func (me *TShaderManager) compileAll () error {
	var err error
	...
	for _, shaderName = range ShaderMan.AllNames {
...
type TShaderSources struct {
	Compute, Fragment, Geometry, TessCtl, TessEval, Vertex map[string]string
}

func (me *TShaderSources) enumerate () map[gl.Enum]int {
...
func (me *TShaderSources) source (glShaderType gl.Enum, shaderName string) string {
	switch glShaderType {
