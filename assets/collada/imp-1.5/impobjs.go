...
func obj_FxTechniqueKind(xn *xmlx.Node, n string) (obj *nga.FxTechniqueKind) {
	if (xn != nil) && (len(n) > 0) {
		xn = xcn(xn, n)
	}
	if xn != nil {
...
func objs_FxTechniqueKind(xn *xmlx.Node, n string) (objs []*nga.FxTechniqueKind) {
	xns := xcns(xn, n)
	objs = make([]*nga.FxTechniqueKind, len(xns))
	for i, xn := range xns {
