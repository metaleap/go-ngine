...
func obj_ParamSidRef(xn *xmlx.Node, n string) (obj *nga.ParamSidRef) {
	if (xn != nil) && (len(n) > 0) {
		xn = xcn(xn, n)
	}
	if xn != nil {
		obj = init_ParamSidRef(xn)
	}
	return
}

func objs_ParamSidRef(xn *xmlx.Node, n string) (objs []*nga.ParamSidRef) {
	xns := xcns(xn, n)
	objs = make([]*nga.ParamSidRef, len(xns))
	for i, xn := range xns {
		objs[i] = obj_ParamSidRef(xn, "")
	}
	return
}
