...
//	
//	1. First, there is a *XyzDef* struct for the one-time definition of a unique resource:
//	GeometryMeshDef, ImageDef, LightDef, MaterialDef etc.
//	
//	2. Next, there is a smaller *XyzInst* struct for handling many individual (sometimes parameterized) instances of
//	an *XyzDef*: GeometryMeshInst, ImageInst, LightInst, MaterialInst etc.
//	
//	3. Finally, there is a light-weight *LibXyzDefs* struct type (based on a *map* collection) containing Defs
//	associated with their ID: LibGeometryMeshDefs, LibImageDefs, LibLightDefs, LibMaterialDefs etc.
//	
//	4. The package also provides a pre-initialized global *XyzDefs* variable for each such Lib type, in simple apps
...
//	*AllXyzDefLibs* variable, essentially a *map* collection of Libs: AllGeometryMeshDefLibs (of type
